...
export async function loadMealCommunicationLedger(supabaseAdmin: any) {
  const [preorders, invitations, inviters, originalSends, updateSends] = await Promise.all([
  ...
  for (const result of [preorders, invitations, inviters, originalSends, updateSends]) {
